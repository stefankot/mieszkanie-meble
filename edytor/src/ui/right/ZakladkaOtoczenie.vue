...
<template>
  <div>
    <div class="flex pb-2.5 pl-(--pad-x) pr-(--pad-prawy)"><Wybor v-model="filtr" :opcje="[{ wartosc: 'all', etykieta: 'All' }, { wartosc: 'sky', etykieta: 'Sky Light' }, { wartosc: 'weather', etykieta: 'Weather' }]" /></div>
    ...
    <Sekcja tytul="Sun">
      ...
      <Pole etykieta="Color Temperature">
        <div class="relative h-(--wys-pola) flex-1 rounded-[2px]" style="background: linear-gradient(90deg, #ff8a00, #ffd9a0 30%, #f4f6ff 50%, #6aa8ff 75%, #2f6bff)">
          <span class="absolute -inset-y-px w-1 rounded-[1px] bg-white ring-1 ring-black/40" :style="{ left: `calc(${pozycjaTemp()}% - 2px)` }" />
        </div>
        <span class="w-10 shrink-0 text-right text-2xs tabular-nums text-text">{{ temperatura }}K</span>
      </Pole>
      <Pole etykieta="Direction">
        <Radio v-model="kierunek" :opcje="[{ wartosc: 'hdri', etykieta: 'Follow HDRI' }, { wartosc: 'custom', etykieta: 'Custom' }]">
          <template #custom>
    ...
    <Sekcja tytul="Weather">
      ...
      <template v-if="mgla">
        <span class="size-[19px] rounded-[2px] bg-white" />
