...
| P36 TRAA zamiast TAAU przy wejściu 100% | (ten commit) | `traa` (A/B: `?bez=traa` = TAAU z P35), `?ostrosc=` | `silnik.js` → `wyjscieDla()`: `traa()` z `depthThreshold` .002 + RCAS 0,4; TAAU zostaje dla PHOTO_RASTER i `?bez=taaupelne`; `sterowanie.js` → etykieta | zgłoszenie: wysoka mniej ostra niż średnia (SMAA). Przyczyna: filtr gaussowski 3×3 rekonstrukcji TAAU. 1036×987, regał: TRAA 36,2 FPS (TAAU 100%: 35,4); zmienność historii 0,037–0,044% w środku, przy krawędziach frontów i z boku (TAAU 0,066%, B2-TRAA 2,2%); z RCAS 0,4 słój ostry jak w SMAA na zrzucie, 35,3 FPS |
| P37 suwaki jakości w panelu | (ten commit) | `suwakijakosci` | `suwaki-jakosci.js` (nowy) → `dodajSuwakiJakosci()`; `sterowanie.js` → `#suwakiJakosci`; `silnik.js` → definicje pól po `potokGotowy`, `wyjscieDla.ostrosc` (uniform), `zastosujNadpisania()` w `ustawPoziomJakosci()` | grupy: Obraz, SSGI, SSR, Cienie, Bloom, Materiały; tylko parametry działające na żywo; zapis w ustawieniach panelu; „auto” = wartość z poziomu jakości |
