...
5. ⏳ W TOKU (przerwane na prośbę użytkownika 17.09 — „zamknij edycję, żeby dało się otworzyć stronę”). Zrobione: `renderery/webgpu/parametryczne.js` — sekcja `parametric` v2: korpus o stałym rozmiarze, siatka kolumny×rzędy z rozkładem equal/fibonacci/random/custom, `definitions` (części z wymiarami względem komórki `{cell:'w',addMm:-4}`, opcjonalny zawias/wysuw) i `instances` (komórki: all / rows+columns / count+order), nadpisania z edytora; 5 testów (`tests/parametryczne.test.mjs`), `parametryczne.d.ts`; `edytor/src/meble/rozklad.ts` korzysta z tego samego algorytmu (vitest 11/11).
   Dalej: (a) `normalizeFurnitureDocument` rozwija `parametric` + walidacja; (b) `biblioteka.przebudujParametryczny(id, nadpisania)` (podmiana korzenia jak przy wersji, zachowanie nadpisań przy odświeżaniu); (c) edytor: `nadpisaniaZUkladu` w projekcji układu (debounce) + ponowne naniesienie materiałów mebla; (d) POC `meble/regal-salon/wersje/v0008-parametric.json` (3074×2430×423) jako nieaktywna wersja + przekazanie `furnitureV2`/`furnitureSource` z adresu edytora do ramki; (e) `FORMAT-MEBLA-V2.md` dla ChatGPT; (f) BatchedMesh — najpierw pomiar draw calls (`renderer.info`), decyzja udokumentowana.
   Uwaga środowiskowa: port 5173 zajął obcy `python3 http.server` (PID 24678, katalog domowy) — edytor uruchomiony na **5174** (`.claude/launch.json` → `edytor-5174`).
