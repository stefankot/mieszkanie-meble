...
<template>
  <div v-if="kropka && zrodlo" class="pointer-events-auto absolute w-[180px] rounded-[4px] bg-[#16181c] p-2.5 shadow-2xl ring-1 ring-white/5" :style="pozycja">
    ...
    <template v-if="kropka.typ === 'ruch'">
      <p class="mb-1.5 text-[10px] text-label">State</p>
      ...
      <p class="mb-1.5 text-[10px] text-label">Handle</p>
    ...
    <template v-else-if="kropka.typ === 'mebel'">
      <p class="mb-1.5 text-[10px] text-label">Doors & drawers</p>
      <div class="mb-2.5 flex gap-2">
      ...
      <p class="mb-1.5 text-[10px] text-label">Palette</p>
    ...
    <template v-else>
      ...
      <p class="mb-1.5 text-[10px] text-label">Color temperature</p>
      <div class="flex gap-2">
        <Kafelek v-for="[k, c] in [['2700', '#ffb46b'], ['3000', '#ffcf8f'], ['4000', '#fff1dc']]" :key="k" :etykieta="k + 'K'" :tlo="c" :wybrany="barwa === k" @wybierz="barwa = k" />
      </div>
    </template>

    <button type="button" class="mt-3 h-[22px] w-full rounded-d5 bg-field text-[10.5px] text-text hover:bg-hover" @click="wiecej">More…</button>
  </div>
</template>
